function nBTCs = jz_calc_hf_PTDD(Img,Ref,B,CF)
% INPUT
...
for r = 1:B:H
    for c = 1:B:W
        ...
        D = dct2(Patch);
        TCs_Ref(r:r+B-1,c:c+B-1) = D;
    end
end

% Calculate Phase 1 Img
TCs_Img = zeros(H,W);
for r = 1:B:H
    for c = 1:B:W
        Patch = Img(r:r+B-1,c:c+B-1);
        D = dct2(Patch);
        D(zz(fix(m/2+1):end)) = 0;
        TCs_Img(r:r+B-1,c:c+B-1) = D;
    end
end

% Replace TCs_Ref by TCs_Img
F = abs(TCs_Img)>0;
TCs = TCs_Img + TCs_Ref.*(1-F);
