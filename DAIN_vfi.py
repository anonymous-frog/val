...
if DO_MiddleBurryOther:
    ...
    for i in range(0,1): 
        ...
        arguments_strFirst  = os.path.join(MB_Other_DATA, "Key_1.png")
        arguments_strSecond = os.path.join(MB_Other_DATA, "Key_2.png")
        ...
        count = 0
        shutil.copy(arguments_strFirst, os.path.join(gen_dir, "{:0>4d}.png".format(count)))
        count  = count+1
        for item, time_offset in zip(y_, time_offsets):
            arguments_strOut = os.path.join(gen_dir, "{:0>4d}.png".format(count))
            count = count + 1
            imsave(arguments_strOut, np.round(item).astype(numpy.uint8))
        shutil.copy(arguments_strSecond, os.path.join(gen_dir, "{:0>4d}.png".format(count)))
        count = count + 1
